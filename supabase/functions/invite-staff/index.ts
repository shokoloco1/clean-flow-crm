...
serve(async (req) => {
  ...
  try {
    ...
    // Note: The profile is already created by the handle_new_user trigger when createUser is called
    // We use upsert to update the existing profile with additional fields
    ...
    if (profileError) {
      console.error("[invite-staff] Profile update error:", profileError);
      throw profileError;
    }

    // 2b. Create/update sensitive profile data (hourly rate)
    if (hourlyRate) {
    ...
    // 3. Assign staff role (upsert to handle re-invitations)
    const { error: roleError } = await supabaseAdmin
      .from("user_roles")
      .upsert({ user_id: userId, role: "staff" }, {
    ...
    // Using upsert to handle re-invitations without errors
    ...
    await supabaseAdmin.from("staff_availability").upsert(availabilityRecords, {

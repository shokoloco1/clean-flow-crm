...
export function useFetchWithRetry<T>(fetchFn: () => Promise<T>, options: UseFetchWithRetryOptions) {
  const { cacheKey, timeout = 5000, maxRetries = 2, retryDelay = 1000 } = options;

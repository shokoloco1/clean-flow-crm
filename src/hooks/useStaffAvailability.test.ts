...
describe("useStaffAvailability", () => {
  ...
  describe("fetchAvailability", () => {
    ...
    it("should use default availability when none exists", async () => {
      ...
      expect(result.current.availability.find((a) => a.day_of_week === 0)?.is_available).toBe(
        false,
      );
    });
  ...
  describe("toggleDay", () => {
    it("should toggle is_available for specified day", async () => {
      ...
      expect(result.current.availability.find((a) => a.day_of_week === 1)?.is_available).toBe(
        false,
      );
    });

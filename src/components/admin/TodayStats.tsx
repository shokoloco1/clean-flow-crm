    ...
    <div className="space-y-2">
      {hasNoJobsToday && (
        <div className="rounded-lg border border-border bg-muted/50 p-3 text-center">
          <p className="text-sm text-muted-foreground">📅 {tAdmin("no_jobs_scheduled_today")}</p>
        </div>
      )}
      <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
        {statItems.map((item) => (
          <div
            key={item.label}
            className="flex items-center gap-3 rounded-lg border border-border bg-card p-3 transition-all hover:shadow-sm"
          >
            ...
            <div>
              <p className="text-lg font-bold tabular-nums text-foreground">
                <AnimatedNumber value={item.value} suffix={item.isPercent ? "%" : ""} />
              </p>

...
export function CSVReports() {
  ...
  const generateJobsReport = async () => {
    ...
    // Fetch related data
    const staffIds = [...new Set(jobs?.filter(j => j.assigned_staff_id).map(j => j.assigned_staff_id))];
    ...
    const [staffResult, propertiesResult] = await Promise.all([
      staffIds.length > 0 
        ? supabase.from('profiles').select('user_id, full_name').in('user_id', staffIds as string[])
        : { data: [] as { user_id: string; full_name: string }[] },
      propertyIds.length > 0
        ? supabase.from('properties').select('id, name, address').in('id', propertyIds as string[])
  ...
  const generateStaffReport = async () => {
    ...
    const { data: jobs, error: jobsError } = await supabase
      .from('jobs')
      .select('id, assigned_staff_id, status, start_time, end_time, scheduled_date, scheduled_time, quality_score')
      .gte('scheduled_date', dateRange.startDate)
      .lte('scheduled_date', dateRange.endDate);
    ...
    const staffStats = profiles?.map(profile => {
      ...
      const earlyDepartures = staffAlerts.filter(a => a.alert_type === 'early_departure').length;

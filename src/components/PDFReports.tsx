...
export function PDFReports() {
  ...
  const generateJobsPDF = async () => {
    ...
    const propertyIds = [
      ...new Set(jobs?.filter((j) => j.property_id).map((j) => j.property_id)),
    ] as string[];
    ...
    doc.setFontSize(10);
    doc.text(
      `Total: ${jobs?.length || 0} | Completed: ${completed} | Pending: ${pending} | In Progress: ${inProgress} | Cancelled: ${cancelled}`,
      14,
      startY + 7,
    );
  ...
  const generateJobDetailPDF = async () => {
    ...
    jobs?.forEach((job, index) => {
      ...
      doc.setFontSize(9);
      doc.text(
        `Date: ${format(new Date(job.scheduled_date), "dd/MM/yyyy")} | Time: ${job.scheduled_time.slice(0, 5)}`,
        14,
        currentY,
      );
      currentY += 5;
      doc.text(
        `Staff: ${staffName} | Duration: ${duration ? `${duration} min` : "N/A"} | Quality: ${job.quality_score || "N/A"}%`,
        14,
        currentY,
      );
      currentY += 5;
      ...
      if (jobChecklist.length > 0) {
        ...
        doc.text(
          `Checklist: ${completed}/${jobChecklist.length} completed, ${issues} issues`,
          14,
          currentY,
        );
        currentY += 5;
  ...
  const reportOptions = [
    {
      ...
      label: "Job History",
      icon: Briefcase,
      description: "Summary of all jobs with status and duration",
    },
    {
      value: "staff",
      label: "Staff Performance",
      icon: Users,
      description: "Statistics and metrics per employee",
    },
    {
      value: "alerts",
      label: "Generated Alerts",
      icon: AlertTriangle,
      description: "Alert history by type and status",
    },
    {
      value: "job-detail",
      label: "Job Details",
      icon: ClipboardCheck,
      description: "Complete report with checklist and photos",
    },
  ];
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>Generate professional reports in PDF format</CardDescription>
      </CardHeader>
